...
export default async function setVTT() {
	const transcriptVTT = get(playingEpisode).transcripts?.find(({ type }) => type.includes('vtt'));
	const transcriptSRT = get(playingEpisode).transcripts?.find(({ type }) => type.includes('srt'));
