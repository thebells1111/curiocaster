...
margin-right: 6px; min-width: 50px"
			/>
			<div class="title" class:selected={$showRecentEpisodes}>Recent Episodes</div>
			{#if $newEpisodesFlag}
		...
		{#each (promoteFeatured ? [$featuredPodcasts.feeds[0]] : []).concat($podcastList.filter((v) => !promoteFeatured || v.id !== $featuredPodcasts.feeds[0].id)) as podcast, index}
			<li
				data-id={podcast.id}
