	...
	async function getEpisodes(key) {
		try {
			...
			let episodeQuery =
				...
				encodeURIComponent(
					`id=${podcast.id}&max=1000&since=${initial ? 1 : podcast.lastUpdateTime}`
				);
			...
			if (channel.status) {
				...
				if (initial) {
					newEpisodes = episodes.items.filter(
						(newEp) => podcast.episodes.findIndex(({ guid }) => newEp.guid === guid) === -1
					);
				...
				allEpisodes.forEach((v) => {
					if (!episodeObject[v.guid]) {
						...
						if (
							v.datePublished >
							Math.floor(new Date().getTime() / 1000) - (recentSettings?.time * 86400 || 604800)
						) {
				...
				recent = recent.concat(recentEpisodes.slice(0, recentSettings?.maxEps || 3));
